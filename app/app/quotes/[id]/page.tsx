...
export default async function QuotePage({ params, searchParams }: {
  ...
  const [{ data: quote }, { data: items }, { data: linkedInvoice }, { data: changeEvents }, { data: revisions }] = await Promise.all([
    ...
    supabase.from('quote_revisions').select('id, version_number, created_at, snapshot').eq('quote_id', id).order('version_number', { ascending: false }).limit(10),
  ])
  ...
  return <QuoteDetail quote={quote} items={items ?? []} autoSend={action === 'send'} linkedInvoiceId={linkedInvoice?.id ?? null} changeRequest={latestChangeRequest ? { message: (latestChangeRequest.meta as { message?: string })?.message ?? '', created_at: latestChangeRequest.created_at } : null} revisions={(revisions ?? []) as { id: string; version_number: number; created_at: string; snapshot: unknown }[]} />
}
